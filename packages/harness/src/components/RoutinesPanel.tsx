...
export const RoutinesPanel: Component<RoutinesPanelProps> = (props) => {
  ...
  const submit = () => {
    ...
    const id = selectedID()
    if (id) props.onUpdate(id, { ...value, name: value.name.trim(), prompt: value.prompt.trim() })
    else props.onAdd({ ...value, name: value.name.trim(), prompt: value.prompt.trim() })
    setEditing(false)
  }
