...
Re-verified 2026-09-21 against `@flupcode/harness` 1.15.3, including the H block (H-13, H-18–H-20,
H-24–H-27, H-29, H-35, H-36) that landed after the HF block.
...
| Editable keybinds, leader key, which-key                      | ✅        | 🟡       | editable keybinds with capture (`keybinds.ts`, H-24); no leader key or which-key         |
...
| Compact / summarize                              | ✅                         | ✅       | `CompactionMarker` renders the divider and summary (`SessionView.tsx`)  |
| Undo / redo with file restore                    | ✅                         | 🟡       | `revert.stage/commit/clear` (redo via clear); no marker in the timeline |
...
| `@agent`, `@mcp-resource`, file contents as a part          | ✅                | 🟡       | `@` menu lists files, agents, artifacts and packs, but inserts text; no `@mcp-resource` or `file` parts |
| Skills as slash commands                                    | ✅                | ✅       | `/name` commands plus the `SkillCatalogue.tsx` manager (H-27)                                           |
| External editor for the prompt, IDE selection context       | ✅                | ❌       | —                                                                                                       |
| Prompt history                                              | ✅ (engine JSONL) | 🟡       | localStorage (`prompt-history.ts`)                                                                      |
| Mobile composer                                             | n/a               | 🟡       | `MobileComposer.tsx` shares `/` and `@` with the desktop composer (H-26); no prompt history             |
...
| Rule editor (`allow`/`ask`/`deny` per tool and pattern) | ✅       | ✅       | `PermissionsPanel.tsx`: per-tool actions and per-pattern rules (H-25) |
...
| Commands with agent/model/variant overrides, subtask      | ✅       | 🟡       | `CommandsPanel.tsx` edits agent/model/subtask (H-25); no variant field                                           |
| Skill manager (sources, per-agent, install)               | ✅       | ✅       | `SkillCatalogue.tsx` + `skill-sources.ts` + `skill-access.ts`: create/edit, folder/URL sources, per-agent (H-27) |
| Editors for agents, commands, permissions, MCP            | ✅       | ✅       | `AgentsPanel`, `CommandsPanel`, `PermissionsPanel`, `McpManager` in Settings (H-13, H-25)                        |
...
| File tree, viewer, text/symbol search | ✅                | 🟡       | `FilesPanel.tsx`: lazy tree, highlighted viewer and ranked path search (H-19); text/symbol search still unused |
| Review panel per git / branch / turn  | ✅                | 🟡       | the Changes panel carries findings as line comments (H-32); no per-turn panel                                  |
| Git: branch and +/-                   | ✅                | ✅       | `ChangesPanel.tsx` + `RepoBar.tsx`: stage by hunk, discard, generated message, branch and PR with CI (H-20)    |
| Worktrees and workspaces              | ✅                | 🟡       | a worktree per writing task via `/experimental/worktree`, with merge and cleanup (H-29); no generic workspaces |
...
| Settings by section with real editors | ✅       | ✅       | `SettingsPanel.tsx` rail with 13 sections and the file-backed editors (H-25); raw JSON is the advanced fallback                                      |
| Themes                                | ✅ (~40) | 🟡       | seven colour themes plus light/dark/system (`tokens.css`); short of upstream's ~40                                                                   |
...
After it, the H block (H-13–H-47, PRs #177–#214) closed the audit's §17 backlog: settings with real
editors (H-25), the file tree and viewer (H-19), real git commits (H-20), the skill manager (H-27),
worktrees per task (H-29), export and share (H-35) and the rest. The only item still blocked is
**F5-4 signing/notarization**, which needs Apple/Windows certificates and CI secrets from outside
the repo.
